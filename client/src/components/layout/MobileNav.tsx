...
export default function MobileNav() {
  ...
  return (
    <div className="bg-blue-800 border-t border-blue-700 shadow-lg text-white w-full md:hidden fixed bottom-0 z-50 flex justify-around py-2 px-1">
      {navItems.map((item) => (
        <Link key={item.path} href={item.path}>
          {isActive(item.path) ? (
            <div className="flex flex-col items-center">
              <div className="w-12 h-12 flex items-center justify-center rounded-full bg-white text-blue-600 shadow-md -mt-6 mb-1 border-2 border-blue-600">
                <i className={`${item.icon} text-lg`}></i>
              </div>
              <span className="text-xs font-bold text-white">{item.label}</span>
            </div>
          ) : (
            <div className="flex flex-col items-center py-1 px-3 cursor-pointer hover:text-blue-300 transition-colors">
              <i className={`${item.icon} text-white opacity-80 text-lg`}></i>
              <span className="text-xs mt-1 text-white opacity-80">{item.label}</span>
            </div>
          )}
        </Link>
      ))}
