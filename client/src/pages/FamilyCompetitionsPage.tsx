...
export default function FamilyCompetitionsPage() {
  ...
  // Query competitions with category=family filter
  const { data: competitions, isLoading } = useQuery<CompetitionWithEntryStatus[]>({
    queryKey: ["/api/competitions/category/family", sortBy],
    queryFn: async () => {
      const response = await fetch(`/api/competitions/category/family?sortBy=${sortBy}`);
      if (!response.ok) {
        throw new Error("Failed to fetch family competitions");
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 text-amber-500">Family Competitions</h1>
        <p className="text-gray-500 dark:text-gray-400">
          Enter exciting competitions to win amazing prizes for your family
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {competitions.map((competition) => (
            <CompetitionCard
              ...
              categoryTheme="family"
            />
          ))}
        </div>
      ) : (
        <div className="text-center py-10">
          <h3 className="text-xl font-medium mb-2">No family competitions found</h3>
          <p className="text-gray-500 dark:text-gray-400">
            Check back soon for new family competitions
          </p>
        </div>
      )}
