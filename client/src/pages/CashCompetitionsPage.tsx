...
export default function CashCompetitionsPage() {
  ...
  // Query competitions with category=cash filter
  const { data: competitions, isLoading } = useQuery<CompetitionWithEntryStatus[]>({
    queryKey: ["/api/competitions/category/cash", sortBy],
    queryFn: async () => {
      const response = await fetch(`/api/competitions/category/cash?sortBy=${sortBy}`);
      if (!response.ok) {
        throw new Error("Failed to fetch cash competitions");
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 text-green-500">Cash Competitions</h1>
        <p className="text-gray-500 dark:text-gray-400">
          Enter competitions to win cash prizes and life-changing jackpots
        </p>
      </div>
      ...
      {competitions && competitions.length > 0 ? (
        ...
        <div className="text-center py-10">
          <h3 className="text-xl font-medium mb-2">No cash competitions found</h3>
          <p className="text-gray-500 dark:text-gray-400">
            Check back soon for new cash competitions
          </p>
        </div>
      )}
